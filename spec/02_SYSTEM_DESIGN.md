...
| FR-002 docker | `docker.Monitor` polls the **privileged helper** on a ticker (10s) and caches the container list; the page and the dashboard read the cache. Since the 2026-09-05 audit (finding C2) the panel holds no access to the Docker daemon socket at all: the helper reads it and exposes list, inspect and logs as read-only actions, and `internal/dockerapi` — the only package that opens that socket — is unreachable from `cmd/ultron-ap`, asserted against the real dependency graph. | In: `/run/ultron-helper.sock`. Out: `[]ContainerInfo` with state, health and stats. | An unreachable helper sets `available=false` and **keeps the last good list**, so a transient failure does not blink the panel to empty and back; the section renders an explicit unavailable state, distinct from the empty state. |
...
| FR-011 privilege separation | Two binaries. `ultron-ap` runs unprivileged, holds no root capability **and no sudo rights whatsoever** — the sudoers profile that once granted it `systemctl`, `shutdown` and a wildcard `pironman5 *` was deleted in BG-079, because it was a parallel privilege path the helper had already made unnecessary. `ultron-helper` runs as root, listens on a Unix socket, enforces `SO_PEERCRED` against a UID allow-list, fails closed when no allow-list resolves, and executes only its allow-listed actions with validated arguments. | In: a JSON request over the socket. Out: a JSON response. | The panel treats an unreachable helper as a degraded state, not a crash. No new host action may bypass this boundary — it is NFR-005, asserted by tests, and a future privileged action goes into the helper's allow-list, never back into sudoers. |
...
## Security Design — NFR mapping (added 2026-09-06)

Phase 1 previously declared no Security NFR at all. The controls below already existed in the
code; what was missing was the requirement recording them, which is why the gate rejected the
artifact. Each is mapped to its design control here.

| NFR | Control | Where |
|---|---|---|
| **NFR-005** privilege separation | Two binaries; helper-only root; `SO_PEERCRED` + UID allow-list, fail-closed with none; per-action allow-list by `switch`; unit names and container ids validated against anchored patterns before use; the panel holds no sudo rights and no daemon socket. | `cmd/ultron-helper`, `internal/privileged`, `internal/dockerapi`, `internal/isolation` |
| **NFR-006** authn / authz / CSRF | bcrypt password check; session cookie HttpOnly + SameSite, Secure behind TLS; `requireAuth` on every route except `/login` and `/health`; per-IP brute-force lockout via atomic UPSERT; per-session CSRF token checked by middleware before any state-changing handler runs. | `internal/auth`, `internal/server/middleware.go` |
| **NFR-007** secret containment | `internal/logfilter` redacts and caps everything crossing the helper boundary; notification credentials encrypted at rest; backups AES-GCM; container env **values** dropped inside the helper so only names cross; no trace ids in assets served verbatim. | `internal/logfilter`, `internal/database/secrets.go`, `backup_crypto.go`, `cmd/ultron-helper/docker.go` |
| **NFR-008** path containment | Admin-supplied backup paths constrained to an allow-listed root and re-validated at use time after symlink resolution, so a path saved as safe cannot be redirected later. | `internal/database/backup_path.go` |

**Trust boundaries.** Untrusted input enters at the browser (session-authenticated, CSRF-checked)
and at the Docker daemon's responses (decoded into narrow concrete types under an
`io.LimitReader`). Privilege changes at exactly one place: the Unix socket between `ultron-ap`
and `ultron-helper`. There is no second crossing, and NFR-005 exists to keep it that way.
