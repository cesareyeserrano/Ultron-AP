// TC-1: Validate us-1 primary behavior
// Acceptance Criteria: AC-1
// AC-1: Given an authenticated administrator on desktop or mobile, when opening the dashboard, then critical indicators are visible within the first viewport, navigation to existing modules is completed in at most 2 taps/clicks, and no regression in baseline resource usage is observed.
import { fr_1_the_system_must_present_the_most_critical_das } from "../../../src/contracts/fr-1-the-system-must-present-the-most.js";
import test from "node:test";
import assert from "node:assert/strict";

test("tc_1_validate_us_1_primary_behavior", async () => {
  const report = await fr_1_the_system_must_present_the_most_critical_das({
    indicatorsInFirstViewport: true,
    navigationInteractions: 2,
    supportsDesktop: true,
    supportsMobile: true,
    baselineRegression: false,
  });
  assert.equal(report.criticalIndicatorsFirstViewport, true);
  assert.equal(report.intuitiveNavigationWithinTwoSteps, true);
  assert.equal(report.mobileAndDesktopSupported, true);
  assert.equal(report.baselinePerformanceRegressed, false);
});
